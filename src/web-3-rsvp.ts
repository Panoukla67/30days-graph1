...
export function handleNewEventCreated(event: NewEventCreated): void {
  ...
  if (newEvent == null) {
    ...
    //newEvent.maxAttendees = event.params.maxAttendees; // maxCapacity to maxAttendees
...
export function handleNewRSVP(event: NewRSVP): void {
  let id = event.params.eventId.toHex() + event.params.attendee.toHex(); //attendeeAddress to attendee
  let newRSVP = RSVP.load(id);
  let account = getOrCreateAccount(event.params.attendee); //attendeeAddress to attendee
